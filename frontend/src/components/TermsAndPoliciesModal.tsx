  ...
  return (
    <AnimatePresence>
      <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 overflow-y-auto bg-slate-950/80 backdrop-blur-md">
        ...
        >
          {/* Header (Stable) */}
          <div className="p-6 border-b border-slate-100 dark:border-slate-800 flex items-center gap-3 bg-gradient-to-r from-slate-50 to-white dark:from-slate-900/50 dark:to-slate-900 shrink-0">
          ...
          {/* Policy content (Scrollable, outline-free to prevent browser highlight overlays) */}
          <div className="p-6 overflow-y-auto max-h-[55vh] flex-grow outline-none focus:outline-none scrollbar-thin scrollbar-thumb-slate-200 dark:scrollbar-thumb-slate-800 border-b border-slate-100 dark:border-slate-800/60">
            <div className="space-y-6">
              {/* Heading */}
              <div>
                <div className="flex gap-2 text-indigo-600 dark:text-indigo-400 items-center mb-3">
                ...
                {/* Structured Paragraphs with Hierarchy */}
                <div className="space-y-4 text-sm text-slate-600 dark:text-slate-300 leading-relaxed pl-6">
                  <p>
              ...
              {/* Warnings Panel */}
              <div className="p-4 rounded-xl bg-amber-50/50 dark:bg-amber-950/10 border border-amber-200/30 dark:border-amber-900/30 flex items-start gap-3 mt-6">
                ...
                <div className="space-y-1">
                  ...
                  <p className="text-xs leading-relaxed text-slate-600 dark:text-slate-300">
                    Users are responsible for ensuring they have the appropriate authorization 
          ...
          {/* Footer Form (Stable, clean checkbox button spacing) */}
          <form onSubmit={handleAccept} className="p-6 bg-slate-50 dark:bg-slate-900/30 shrink-0 flex flex-col gap-4">
            {error && (
            ...
            {/* Accessible checkbox block */}
            <label className="flex items-start gap-3 cursor-pointer group p-3 rounded-xl hover:bg-slate-100/50 dark:hover:bg-slate-800/30 transition-colors border border-transparent hover:border-slate-200/30 dark:hover:border-slate-750/20">
              <input
                type="checkbox"
                checked={isChecked}
                onChange={(e) => setIsChecked(e.target.checked)}
                className="mt-0.5 w-4 h-4 rounded border-slate-300 dark:border-slate-700 text-indigo-600 focus:ring-indigo-500/30 dark:bg-slate-800 cursor-pointer"
                required
              />
              <span className="text-xs text-slate-600 dark:text-slate-400 select-none group-hover:text-slate-800 dark:group-hover:text-slate-200 transition-colors leading-normal">
                I confirm that I have read and agree to the Terms of Use and consent to the processing of my data as described above.
              </span>
            </label>

            {/* Action buttons with explicit states */}
            <div className="flex items-center justify-end pt-2">
              <button
                type="submit"
                disabled={!isChecked || loading}
                className={`w-full sm:w-auto px-6 py-2.5 font-semibold rounded-xl transition-all duration-300 flex items-center justify-center gap-2 shadow-lg ${
                  !isChecked
                    ? 'bg-slate-200 dark:bg-slate-800 text-slate-400 dark:text-slate-600 cursor-not-allowed border border-slate-300/10 shadow-none pointer-events-none'
                    : 'bg-gradient-to-r from-indigo-600 via-purple-600 to-pink-600 hover:from-indigo-700 hover:via-purple-700 hover:to-pink-700 text-white shadow-indigo-500/25 hover:shadow-indigo-500/40 cursor-pointer'
                }`}
              >
                {loading ? (
                  <>
